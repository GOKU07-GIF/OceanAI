...
# Use the Windows/OS certificate store while keeping HTTPS verification enabled.
...
class INCOISERDDAPSSTProvider:
    """INCOIS ERDDAP adapter for historical ARGO-derived SST context.

    The weekly INCOIS SST dataset is used only as observational/context
    evidence. Current and future SST should come from an operational source.
    """
    ...
    def fetch(
        ...
        if latitude is None or longitude is None:
            return {
                "status": "unavailable",
                "error": (
                    "Latitude and longitude are required for the "
                    "INCOIS SST lookup."
                ),
            }
        ...
        if not isinstance(table, dict):
            return {
                "status": "unavailable",
                "error": (
                    "INCOIS ERDDAP returned an unexpected response shape."
                ),
            }
        ...
        if (
            ...
            return {
                "status": "unavailable",
                "error": (
                    "No INCOIS SST observation was returned for the "
                    "requested location."
                ),
            }
        ...
        if not isinstance(first_row, list):
            return {
                "status": "unavailable",
                "error": (
                    "INCOIS ERDDAP returned an invalid row payload."
                ),
            }
        ...
        if not isinstance(sst_value, (int, float)):
            return {
                "status": "unavailable",
                "error": (
                    "INCOIS returned a missing/non-numeric SST value."
                ),
            }
        ...
        data: dict[str, Any] = {
            ...
            "metadata": {
                ...
                "note": (
                    "Historical INCOIS ARGO SST context; not a current "
                    "or future forecast."
                ),
            },
        }
